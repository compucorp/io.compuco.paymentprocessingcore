...
class Civi_Paymentprocessingcore_Service_InstalmentGenerationServiceTest extends BaseHeadlessTest {
  ...
  public function testGeneratesInstalmentForDueRecurring(): void {
    ...
    $this->createContribution($contactId, $recurId, [
      ...
      'receive_date' => date('Y-m-d', strtotime('-1 month')),
    ]);
    ...
    // Verify the new contribution exists with correct fields from template.
    $contributions = \Civi\Api4\Contribution::get(FALSE)
      ->addSelect('contribution_status_id:name', 'contact_id', 'total_amount', 'currency', 'contribution_recur_id')
  ...
  public function testBatchSizeLimitsProcessing(): void {
    ...
    for ($i = 0; $i < 3; $i++) {
      ...
      $this->createContribution($contactId, $recurId, [
        'contribution_status_id:name' => 'Completed',
        'receive_date' => date('Y-m-d', strtotime('-2 months')),
      ]);
    }
  ...
  public function testContinuesOnSingleRecordError(): void {
    ...
    // Create a valid recurring contribution with template.
    ...
    // Create a recurring contribution without template (will cause error).
    $this->createRecurringContribution($contactId, $processorId, [
      'contribution_status_id:name' => 'In Progress',
  ...
  public function testCreateInstalmentReturnsContributionId(): void {
    ...
    $contributionId = $this->service->createInstalment($recurId, $receiveDate);
  ...
  /**
   * Tests createInstalment throws when no template contribution exists.
   */
  public function testCreateInstalmentThrowsWhenNoTemplate(): void {
    $processorId = $this->createPaymentProcessor();
    $contactId = $this->createContact();
    $recurId = $this->createRecurringContribution($contactId, $processorId);

    $this->expectException(\RuntimeException::class);
    $this->service->createInstalment($recurId, date('Y-m-d'));
  }
